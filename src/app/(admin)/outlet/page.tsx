...
export default async function OutletPage() {
  const [outlets, partnerships] = await Promise.all([
  ...
  const rows: OutletRow[] = outlets.map((c) => ({
    id: c.id,
    name: c.name,
    email: c.email,
    banned: c.banned,
    ...
    partnershipLogo: c.outletLogo ?? c.subPartnership?.logo ?? c.partnership?.logo ?? null,
    outletAddress: c.outletAddress,
    outletPhone: c.outletPhone,
    outletPic: c.outletPic,
    outletLogo: c.outletLogo,
  ...
  return <OutletManager outlets={rows} partnerships={partnershipOptions} />;
}
